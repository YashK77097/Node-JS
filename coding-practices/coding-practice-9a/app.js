...
app.post("/register", async (request, response) => {
  ...
  const hashedPassword = await bcrypt.hash(password, 10); //password will encrypted into 10 characters.
...
app.post("/login", async (request, response) => {
  ...
  const checkUserIdQuery = `
    ...
    WHERE username = '${username}';
  `;
  const userId = await db.get(checkUserIdQuery);
  if (userId === undefined) {
    //checking user is already registered and if not then as following:
    response.status(400);
    response.send("Invalid User!!!");
  } else {
    const isPasswordMatched = await bcrypt.compare(password, userId.password);
    ...
    if (isPasswordValid === true) {
      if (newPassword.length > 5) {
        ...
        const updatePasswordQuery = `
              ...
              where username = '${username}'
            `;
        const updatedPasswd = await db.run(updatePasswordQuery);
        response.status(200);
